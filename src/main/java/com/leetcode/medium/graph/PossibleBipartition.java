...
//        System.out.println(possibleBipartition(10, new int[][] {{4,7},{4,8},{5,6},{1,6},{3,7},{2,5},{5,8},{1,2},{4,9},{6,10},{8,10},{3,6},{2,10},{9,10},{3,9},{2,3},{1,9},{4,6},{5,7},{3,8},{1,8},{1,7},{2,4}}));
    ...
    public static boolean possibleBipartition(int N, int[][] dislikes) {
        ...
        if(N < 2 || dislikes == null || dislikes.length == 0) return true;
        int[] l1 = new int[N];
        int[] l2 = new int[N];
        List<List<Integer>> outRelation = new ArrayList<>(N);
        for (int i = 0; i < N; i++) {
            l1[i] = 1;
            l2[i] = 1;
            outRelation.add(new ArrayList<>());
        }

        for (int i = 0; i < dislikes.length; i++) {
            int[] dislike = dislikes[i];
            outRelation.get(dislike[0] - 1).add(dislike[1] - 1);
        }

        for (int i = 0; i < outRelation.size() ; i++) {
            if(l2[i] == 0) {
                int[] temp = l2;
                l2 = l1;
                l1 = temp;
            }
            for (Integer inDegree : outRelation.get(i)) {
                l1[i] = 0;
                l2[inDegree] = 0;
            }
        }

        return Arrays.stream(l1).sum() + Arrays.stream(l2).sum() >= N;
    }
}
